"""ApproveDepartment Lambda resolver for AppSync.
...
def handler(event, context):
    try:
        return approve_department(event)
    except ForbiddenError as exc:
        raise Exception(exc.message) from exc


def approve_department(event):
    """Approve a Pending_Approval department. Validates: Requirements 4.1, 4.7, 4.8, 4.9"""
    ...
    if existing["approval_status"] != "Pending_Approval":
        raise ValueError(
            f"Cannot approve department with approval_status "
            f"'{existing['approval_status']}'. "
            f"Only Pending_Approval departments can be approved"
        )
